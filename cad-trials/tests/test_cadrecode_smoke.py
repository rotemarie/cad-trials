...
pytestmark = pytest.mark.skipif(
    shutil.which("nvidia-smi") is None or not os.environ.get("CADRECODE_WEIGHTS_OK"),
    reason="needs GPU + cad-recode weights")


def test_one_point_cloud_produces_a_row(tmp_path):
    import trimesh
    from cad_trials.common.meshes import normalize_mesh, sample_point_cloud
    from cad_trials.models.cadrecode import main
    ...
    rows = load_runs(runs)
    assert len(rows) == 1 and rows[0].model == "cadrecode"
